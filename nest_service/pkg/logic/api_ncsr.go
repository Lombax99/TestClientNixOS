...
func sendCSR(csr *models.NebulaCsr, option int) (*models.RawCaResponse, error) {
	...
	if utils.Eidolon {
		respStatus, err := http.Get("http://" + utils.Eidolon_keep_service_ip + ":" + utils.Eidolon_ca_service_port + "/blacklist/verify/" + csr.Hostname)
		if err != nil {
			return nil, err
		}
		...
		if status != "safe" {
			ca_service_ip = utils.Eidolon_keep_service_ip
