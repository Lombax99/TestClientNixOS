...
var (
	...
	//The NEST CONFIG service IP address on the NEST system Nebula network
	Eidolon_keep_service_ip string = "192.168.80.4"

	//The NEST CONFIG service port
	Eidolon_ca_service_port string = "31313"
	...
	Dhall_configuration string = "nebula/nebula_conf.dhall"

	//Folder containing the NEST clients Nebula configuration files generated by this service
	Conf_gen_dir string = "nebula/generated/"
	...
	//Folder containing the blacklist file (added by the Eidolon Network project)
	Blacklist_folder string = "blacklist/"

	//Bolean to enable or disable the Eidolon Keep interaction
	Eidolon bool = false
)
...
func SetupNebula(nebula_folder string, nebula_log *os.File) error {
	...
	if !IsExecOwner(info.Mode()) {
		// this change the file mode to be executable, it might be a security issue so its imperative that the file is safe and not changed by an attacker
		os.Chmod(nebula_folder+"nebula", 0700)
	}

	// start nebula, stdout and stderr are redirected to the log file provided
	println(nebula_folder+"nebula", "-config", nebula_folder+"config.yml")
	...
	// check if the nebula interface is up and running
	interfaces, err := net.Interfaces()
	if err != nil {
		fmt.Printf("Couldn't check information about host interfaces\n")
		return err
	}
